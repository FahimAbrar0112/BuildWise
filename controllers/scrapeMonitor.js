...
exports.scrapeStartech = async () => {
    ...
    for (link of links) {
        try {
            ...
            try {
                await connection.execute('INSERT INTO monitor_shop (productName, price, shop, link, warranty) VALUES (?, ?, ?, ?, ?)', [productName[0], price[0], 'Startech', link, warranty]);
            } catch (error) {
                console.error(error);
            }
        }
...
exports.scrapeTechland = async () => {
    ...
    for (link of links) {
        try {
            ...
            description = await page.$$eval('#blocks-663-1-tab-2 > div > div > div', (elements) => {
                return elements.map((element) => {
                    // Use String.replace() to remove newline characters
                    return element.textContent.trim();
                });
...
exports.scrapePcHouse = async () => {
    ...
    for (link of links) {
        try {
            ...
            description = await page.$$eval('#blocks-65539a5ad7c07-tab-2 > div > div', (elements) => {
                return elements.map((element) => {
                    // Use String.replace() to remove newline characters
                    return element.textContent.trim();
                });
